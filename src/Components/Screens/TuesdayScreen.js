...
const cores = ["#2E7D32", "#f73131"];
const days = ["TercaAmbiental", "TercaUniversitario"];
const logos = [logo1, logo2];

export default class TuesdayScreen extends Component {
  state = {
    categoria1: "ambientalDiaUm",
    categoria2: "universitarioDiaUm",
    statusAmbiental: "",
    statusUniversitario: ""
  };
  ...
  votingStatusAmbiental = () => {
    axios
      .get("https://cpc-backend.herokuapp.com/votingTime/ambientalTer")
      .then(res => {
  ...
  votingStatusUniversitario = () => {
    axios
      .get("https://cpc-backend.herokuapp.com/votingTime/universitariaTer")
      .then(res => {
    ...
    switch (this.state.statusAmbiental) {
      case 1:
        AsyncStorage.getItem(categoria).then(categoria => {
          if (!categoria) {
            this.props.navigation.navigate("TercaAmbiental");
  ...
  handleAlreadyVote1 = categoria => {
    switch (this.state.statusUniversitario) {
      case 1:
        AsyncStorage.getItem(categoria).then(categoria => {
          if (!categoria) {
            this.props.navigation.navigate("TercaUniversitario");
          } else if (categoria === "true") {
            Alert.alert(
              "Aviso",
              "Você já votou nesta categoria.",
