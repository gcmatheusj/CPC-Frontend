...
import { View, Text, StyleSheet, ScrollView, AsyncStorage, Alert } from "react-native";
import { NavigationActions, StackActions } from "react-navigation";
import { Button, ListItem, Left, Right, Radio, Item, Input, Spinner } from "native-base";
...
export default class FridayBrasileiroScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      movieSelected: "",
      category: "Festival do Cinema Brasileiro",
      userName: "",
      token: "",
      password: "",
      isFetching: ""
  ...
  setVotacao = () => {
    const vote = "true";
    AsyncStorage.setItem("brasileiroDiaQuatro", vote);
  };
            ...
            return (
              <ListItem style={styles.item} key={key}>
                ...
                <Right>
                  <Radio
                    onPress={() => {
                      this.setState({ movieSelected: selected[key] });
                    }}
                    selected={this.state.movieSelected === selected[key]}
                    selectedColor="#2a80dd"
                  />
                </Right>
              </ListItem>
            );
